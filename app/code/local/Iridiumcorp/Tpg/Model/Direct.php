...
class Iridiumcorp_Tpg_Model_Direct extends Mage_Payment_Model_Method_Abstract
{
	...
	*/  
	...
	public function _runTransaction(Varien_Object $payment, $amount)
	{
		...
		$iclISOCurrencyList = TPG_ISOCurrencies::getISOCurrencyList();
		$iclISOCountryList = TPG_ISOCountries::getISOCountryList();
		
		$rgeplRequestGatewayEntryPointList = new TPG_RequestGatewayEntryPointList();
		...
		$cdtCardDetailsTransaction = new TPG_CardDetailsTransaction($rgeplRequestGatewayEntryPointList);
		...
		{
			$nCurrencyCode = new TPG_NullableInt($icISOCurrency->getISOCode());
		...
		else
		{
			...
			switch ($cdtrCardDetailsTransactionResult->getStatusCode())
			{
				...
				case 5:
                    ...
                    $error = Iridiumcorp_Tpg_Model_Common_GlobalErrors::ERROR_182;
				...
				case 30:
					...
					$error = Iridiumcorp_Tpg_Model_Common_GlobalErrors::ERROR_183;
                    $error .= $szNotificationMessage;
	...
	public function _runTransparentRedirectTransaction(Varien_Object $payment, $amount)
	{
		...
		$szHashDigest = TPG_PaymentFormHelper::calculateTransparentRedirectHashDigest($szMerchantID, $szPassword, $hmHashMethod, $szPreSharedKey, $nAmount, $nCurrencyCode, $szOrderID, $szTransactionType, $szTransactionDateTime, $szCallbackURL, $szOrderDescription);
	...
	public function _run3DSecureTransaction(Varien_Object $payment, $szPaRes, $szMD)
	{
		...
		$tdsaThreeDSecureAuthentication = new TPG_ThreeDSecureAuthentication($rgeplRequestGatewayEntryPointList);
		...
		{
			$szNotificationMessage = "Payment Processor Response: ".$tdsarThreeDSecureAuthenticationResult->getMessage();
			...
			switch ($tdsarThreeDSecureAuthenticationResult->getStatusCode())
			{
				...
				case 5:
					// status code of 5 - means transaction declined
					$message = Iridiumcorp_Tpg_Model_Common_GlobalErrors::ERROR_183;
                    $message .= $szNotificationMessage;
					$_SESSION['tpg_message'] = $message;
					$error = true;
					break;
				case 20:
					// status code of 20 - means duplicate transaction
	...
	public function _runRedirectedPaymentComplete(Varien_Object $payment, $boIsHostedPaymentAction, $szStatusCode, $szMessage, $szPreviousStatusCode, $szPreviousMessage, $szOrderID, $szCrossReference)
	{
		...
		switch ($szStatusCode)
    	{
    		...
    		case "5":
                ...
                $error = true;
                //$message = Iridiumcorp_Tpg_Model_Common_GlobalErrors::ERROR_183 . "<br />" . $message;    
                break;            
            case "30":
    		default:
    			Mage::log($szWording."transaction couldn't be completed. ".$message);
	...
	*/
    ...
    private function _runCrossReferenceTransaction(Varien_Object $payment, $szTransactionType, $amount = false)
    {
		...
		if($error === false)
		{
	    	...
	    	$crtCrossReferenceTransaction = new TPG_CrossReferenceTransaction($rgeplRequestGatewayEntryPointList);
