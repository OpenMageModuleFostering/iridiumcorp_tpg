...
class Iridiumcorp_Tpg_PaymentController extends Mage_Core_Controller_Front_Action
{
				...
				$szErrorMessage = curl_error($cCurl);
	   			...
	   			else
	   			{
	   				...
	   				catch(Exception $exc)
	   				{
						$boError = true;
			    		$szPaymentProcessorResponse = $exc->getMessage();
                        $szErrorMessage = Iridiumcorp_Tpg_Model_Common_GlobalErrors::ERROR_183;
			    		Mage::logException($exc);
	   				}
	   			}
	   		}
    ...
    private function _postThreeDSecureAuthentication($szPassword, $hmHashMethod, $szPreSharedKey)
    {
    	...
    	$szHashDigest = TPG_PaymentFormHelper::calculatePostThreeDSecureAuthenticationHashDigest($szMerchantID, $szPassword, $hmHashMethod, $szPreSharedKey, $szPaRES, $szCrossReference, $szTransactionDateTime, $szCallbackURL);
